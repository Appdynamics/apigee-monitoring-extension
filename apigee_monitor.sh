...
#Author : Israel.Ogbole@appdynamics.com & Stuart Greenshields
version="[ApigeeMonitore v21.6.1  Build Date 2021-06-23]"
...
if [ "${timezone}" = "saas" ]; then
  ...
  ## MAC TESTING
  #apiquery_minutes_ago=$(date -u -r $(( $(date +%s) - ${query_interval_in_secs} )) | awk '{print $4}')
  #apiquery_to_minutes_ago=$(date -u -r $(( $(date +%s) - ${apigee_query_delay_secs} )) | awk '{print $4}')
  #apiquery_from_minutes_ago=$(date -u -r $(( $(date +%s) - ${apigee_from_in_secs} )) | awk '{print $4}')
  ...
  # #MAC TESTING
  #apiquery_minutes_ago=$(date -r $(( $(date +%s) - ${query_interval_in_secs} )) | awk '{print $4}')
  #apiquery_to_minutes_ago=$(date -r $(( $(date +%s) - ${apigee_query_delay_secs} )) | awk '{print $4}')
  #apiquery_from_minutes_ago=$(date -r $(( $(date +%s) - ${apigee_from_in_secs} )) | awk '{print $4}')
...
## UNCOMMENT FOR WHEN TESTING ON #MAC
#time_now=$(date +"%T")
#today=$(date +"%m/%d/%Y")
#from_range=$(echo ${today}+${apiquery_from_minutes_ago})
#to_range=$(echo ${today}+${apiquery_to_minutes_ago})
